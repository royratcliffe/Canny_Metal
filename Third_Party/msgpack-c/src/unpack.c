...
static int template_execute(template_context *ctx, const char *data, size_t len, size_t *off);
...
static inline int template_callback_array(unpack_user *u, unsigned int n, msgpack_object *o) {
  ...
  if (n > SIZE_MAX / sizeof(msgpack_object)) return MSGPACK_UNPACK_NOMEM_ERROR;
#endif
...
#if SIZE_MAX == UINT_MAX
  if (n > SIZE_MAX / sizeof(msgpack_object_kv)) return MSGPACK_UNPACK_NOMEM_ERROR;
#endif
...
static inline int template_callback_str(unpack_user *u, const char *b, const char *p, unsigned int l,
                                        msgpack_object *o) {
  MSGPACK_UNUSED(b);
  if (*u->z == NULL) {
    *u->z = msgpack_zone_new(MSGPACK_ZONE_CHUNK_SIZE);
...
static inline int template_callback_bin(unpack_user *u, const char *b, const char *p, unsigned int l,
                                        msgpack_object *o) {
  MSGPACK_UNUSED(b);
  if (*u->z == NULL) {
    *u->z = msgpack_zone_new(MSGPACK_ZONE_CHUNK_SIZE);
...
static inline void init_count(void *buffer) { *(volatile _msgpack_atomic_counter_t *)buffer = 1; }
...
bool msgpack_unpacker_expand_buffer(msgpack_unpacker *mpac, size_t size) {
  if (mpac->used == mpac->off && get_count(mpac->buffer) == 1 && !CTX_REFERENCED(mpac)) {
    // rewind buffer
...
static inline msgpack_unpack_return unpacker_next(msgpack_unpacker *mpac, msgpack_unpacked *result) {
  int ret;
...
msgpack_unpack_return msgpack_unpacker_next_with_size(msgpack_unpacker *mpac, msgpack_unpacked *result,
                                                      size_t *p_bytes) {
  msgpack_unpack_return ret;
